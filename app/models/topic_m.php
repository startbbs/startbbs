...
class topic_m extends SB_Model
{
	...
	获取栏目条目
	*/
	public function count_topics($cid)
	{
	...
	/*贴子列表页带分页*/
	public function get_topics_list ($page, $limit, $cid)
	{
	...
	/**/
	public function get_topics_list_by_cids ($limit, $cids)
	{
		$sql="SELECT * from ( SELECT `a`.`topic_id`, `a`.`title`, `a`.`cid`, `a`.`updatetime`, `b`.`uid`, `b`.`username` FROM (`{$this->db->dbprefix}topics` a) LEFT JOIN `{$this->db->dbprefix}users` b ON `b`.`uid` = `a`.`uid` WHERE `cid` IN ({$cids}) ORDER BY `a`.`updatetime` DESC LIMIT {$limit}) alias GROUP BY cid";
$query=$this->db->query($sql);
		//备用
		//$this->db->select('a.topic_id,a.title,a.cid,a.updatetime,b.uid,b.username');
		//$this->db->from('topics a');
		//$this->db->join('users b','b.uid = a.uid','left');
		...
		//$sql="SELECT `a`.`topic_id`, `a`.`title`, `a`.`cid`, `a`.`updatetime`, `b`.`uid`, `b`.`username` FROM (`sb_topics` a) LEFT JOIN `sb_users` b ON `b`.`uid` = `a`.`uid` WHERE `cid` IN ('10', '9', '8', '6', '4', '11', '7', '5') ORDER BY `a`.`updatetime` desc LIMIT 8";
		//$query=$this->db->query($sql);
	...
	{
		$this->db->select('a.topic_id, a.title, a.addtime, a.views, a.uid, a.comments, a.is_top, a.is_hidden, b.cname, b.cid, c.username');
		$this->db->from('topics a');
	...
	public function get_near_id($topic_id,$cid,$position)
	{
		if($position==0){
