...
class Forum extends SB_controller
{
	...
	public function view ($fid=1,$page=1)
	{
		...
		if ($openid) {
			setcookie("openid",$openid,time()+3600*240,'/bbs');
			$user = $this->db->select('uid,username,password,openid,group_type,gid')->from('users')->where('openid', $openid)->limit(1)->get()->row_array();
			if($user){
				$this->session->set_userdata(array ('uid' => $user['uid'], 'username' => $user['username'],'openid'=>$openid,'password' => $user['password'], 'group_type' => $user['group_type'], 'gid' => $user['gid']));
				redirect('/');
			}
		}
			...
			if($_POST && $this->validate_add_form()){
				...
				foreach ( $matches [1] as $u ) {
					if ($u) {
						...
						if ($res['uid']) {
							// 在这里记录下uid，下面20行左右把提醒信息入库
							$notification_uids[] = $res['uid'];
				...
				if($this->forum_m->add($data)){
					...
					//入notifications 表
					if(is_array($notification_uids)){
						//@提醒别人，不提醒自己
						$this->load->model('notifications_m');
						foreach ($notification_uids as $uid) {
							$this->notifications_m->notice_insert($new_fid,$this->uid,$uid,2);
							//更新接收人的提醒数
							$this->db->set('notices','notices+1',FALSE)->where('uid', $uid)->update('users');
						}
					}
	...
	private function validate_add_form(){
		...
		$this->form_validation->set_rules('title', '标题' , 'trim|required|strip_tags|htmlspecialchars|min_length[2]|max_length[80]');
		$this->form_validation->set_rules('content', '内容' , 'trim|max_length['.$this->config->item('words_limit').']');
	...
	public function edit($fid)
	{
		...
		if(!$this->auth->is_login()) {
			...
			if($_POST && $this->validate_add_form()){
				...
				if($this->config->item('auto_tag') =='on'){
					// 自动获取关键词tag
					$str['keywords'] = $this->tag_m->get_tag_auto(strip_tags($str['title']), strip_tags($str['content']));
				} else{
					$str['keywords'] = $this->input->post ('keywords', true);
				}
				//入tag表
				$this->tag_m->insert_tag($str['keywords'], $fid);
				...
				foreach ( $matches [1] as $u ) {
					if ($u) {
						...
						if ($res['uid']) {
							...
							$replace [] = '<a target="_blank" href="'.site_url('user/info/'.$res['uid']).'" >@' . $u . '</a>';
							if($this->uid != $res['uid']){
								$this->notifications_m->notice_insert($fid,$this->uid,$res['uid'],2);
								$this->db->set('notices','notices+1',FALSE)->where('uid', $res['uid'])->update('users');
							}
						}
					}
				}
				$str['content'] = str_replace( @$search, @$replace, $comment);
	...
	public function del($fid,$cid,$uid)
	{
		...
		if($this->auth->is_admin() || $this->auth->is_master($cid)){
			...
			$this->myclass->notice('if(confirm("确定要删除此话题吗！")) {window.history.go(-1); }');
			//删除贴子及它的回复
